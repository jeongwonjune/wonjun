...
class ABMLP(torch.nn.Module):
    def __init__(self, input_dim=2, hidden_dim=32):
        super(ABMLP, self).__init__()
        self.fc1 = torch.nn.Linear(input_dim, hidden_dim)
        self.fc2 = torch.nn.Linear(hidden_dim, hidden_dim // 2)
        self.fc3 = torch.nn.Linear(hidden_dim // 2, 1)  # Output scalar α
        self.sigmoid = torch.nn.Sigmoid()  # Ensure α is in [0, 1]

    def forward(self, loss_bce, loss_ce):
        # Detach the losses so they don't propagate gradients to the original model
        loss_bce = loss_bce.detach()
        loss_ce = loss_ce.detach()

        x = torch.stack([loss_bce, loss_ce], dim=-1)  # Combine losses into a tensor
        x = F.relu(self.fc1(x))
        x = F.relu(self.fc2(x))
        alpha = self.sigmoid(self.fc3(x))  # Output balancing weight
        return alpha
