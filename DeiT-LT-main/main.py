...
from teacher_models.resnet_imagenet import resnet152
...
def main(args):
    ...
    cls_weight = 1.0 / (np.array(cls_num_list) ** 0.2)
    cls_weight = cls_weight / np.sum(cls_weight) * len(cls_num_list)
    samples_weight = np.array([cls_weight[t] for t in dataset_train.targets])
    ...
    if True:  # args.distributed:
        ...
        if args.repeated_aug:
            ...
            sampler_train = torch.utils.data.WeightedRandomSampler(samples_weight, len(samples_weight),replacement=True)
        else:
    ...
    if args.distillation_type != "none":
        ...
        if "regnet" in args.teacher_model:
            ...
            if not args.paco:
                ...
                if args.teacher_model == 'resnet152':
                    teacher_model = resnet152(num_classes=args.nb_classes)
                  
                if args.teacher_model == 'vit'   :
                    teacher_model = create_model('vit_base_patch16_224', num_classes=args.nb_classes)
        ...
        elif args.teacher_model == 'resnet152' or args.teacher_model == 'vit':
            state_dict = checkpoint['state_dict']
            new_state_dict = {k.replace('module.', '') if 'module.' in k else k: v for k, v in state_dict.items()}
    ...
    for epoch in range(start_epoch, epochs):
        ...
        if (
            ...
            if not args.no_distillation:
                if args.weighted_baseloss:
                    ...
                    if args.bce_loss:
                        base_criterion = torch.nn.BCEWithLogitsLoss(weight=per_cls_weights)
                        #base_criterion = torch.nn.CrossEntropyLoss( weight=per_cls_weights)
                    else:
        ...
        mlp = ABMLP()
        mlp.to(device)
        balance_optimizer = torch.optim.Adam(mlp.parameters(), lr = 0.01)
