...
def train_one_epoch(
    ...
    balancing_model: torch.nn.Module,
    ...
    for data_iter_step, (samples_student, targets) in enumerate(
        ...
        if args.bce_loss:
            if epoch >= drw:
                targets_student = torch.nn.functional.one_hot( targets_student.to(torch.int64), num_classes=args.nb_classes).to(torch.float32).cuda()
        ...
        with torch.cuda.amp.autocast():
            ...
            if not args.no_distillation:
                ...
                #alpha = balancing_model(cls_loss, dst_loss)
                #loss = (1-alpha)*cls_loss + alpha*dst_loss
                
                #balance_loss = F.mse_loss(alpha , cls_loss/(cls_loss+dst_loss))
                #balance_optimizer.zero_grad()
                #balance_loss.backward(retain_graph=True)
