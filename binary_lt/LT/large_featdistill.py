...
def main_worker(gpu, ngpus_per_node, args):
    ...
    if args.batch_size <= 128:
        if args.scheduler == 'cosann':
            enc_scheduler = torch.optim.lr_scheduler.CosineAnnealingLR(enc_optimizer, T_max=args.epochs)
            fc_scheduler = torch.optim.lr_scheduler.CosineAnnealingLR(fc_optimizer, T_max=args.epochs)
            feat_conv_scheduler = torch.optim.lr_scheduler.CosineAnnealingLR(feat_conv_optimizer, T_max=args.epochs)
            fc_factor_scheduler = torch.optim.lr_scheduler.CosineAnnealingLR(fc_factor_optimizer, T_max=args.epochs)
            enc_factor_scheduler = torch.optim.lr_scheduler.CosineAnnealingLR(enc_factor_optimizer, T_max=args.epochs)
        elif args.scheduler == 'cosannwr':
            after_enc_scheduler = torch.optim.lr_scheduler.CosineAnnealingLR(enc_optimizer, T_max=args.epochs)
            after_fc_scheduler = torch.optim.lr_scheduler.CosineAnnealingLR(fc_optimizer, T_max=args.epochs)
            after_feat_conv_scheduler = torch.optim.lr_scheduler.CosineAnnealingLR(feat_conv_optimizer, T_max=args.epochs)
            after_fc_factor_scheduler = torch.optim.lr_scheduler.CosineAnnealingLR(fc_factor_optimizer, T_max=args.epochs)
            after_enc_factor_scheduler = torch.optim.lr_scheduler.CosineAnnealingLR(enc_factor_optimizer, T_max=args.epochs)

            enc_scheduler = GradualWarmupScheduler(enc_optimizer, multiplier=1, total_epoch=5, after_scheduler=after_enc_scheduler)
            fc_scheduler = GradualWarmupScheduler(enc_optimizer, multiplier=1, total_epoch=5,
                                                   after_scheduler=after_fc_scheduler)
            feat_conv_scheduler = GradualWarmupScheduler(feat_conv_optimizer, multiplier=1, total_epoch=5,
                                                  after_scheduler=after_feat_conv_scheduler)
            fc_factor_scheduler = GradualWarmupScheduler(fc_factor_optimizer, multiplier=1, total_epoch=5,
                                                  after_scheduler=after_fc_factor_scheduler)
            enc_factor_scheduler = GradualWarmupScheduler(enc_factor_optimizer, multiplier=1, total_epoch=5,
                                                         after_scheduler=after_enc_factor_scheduler)
    else:
        ...
        enc_scheduler = GradualWarmupScheduler(enc_optimizer, multiplier=1, total_epoch=5,
                                               after_scheduler=after_enc_scheduler)
        fc_scheduler = GradualWarmupScheduler(enc_optimizer, multiplier=1, total_epoch=5,
                                              after_scheduler=after_fc_scheduler)
        feat_conv_scheduler = GradualWarmupScheduler(feat_conv_optimizer, multiplier=1, total_epoch=5,
                                                     after_scheduler=after_feat_conv_scheduler)
...
def train(train_loader, teacher_model, lws_model, student_enc, student_fc,
    ...
    for i, (images, target) in enumerate(train_loader):
        ...
        # enc_kl_losses.update(enc_kl_loss.item() if enc_kl_loss is not 0 else 0, s_out.size(0))
        # enc_feat_losses.update(enc_feat_loss.item() if enc_feat_loss is not 0 else 0, s_out.size(0))
        # fc_kl_losses.update(fc_kl_loss.item() if fc_kl_loss is not 0 else 0, s_out.size(0))
        enc_kl_losses.update(enc_kl_loss.item(), s_out.size(0))
        enc_feat_losses.update(enc_feat_loss.item(), s_out.size(0))
        fc_kl_losses.update(fc_kl_loss.item(), s_out.size(0))
