...
def main():
    ...
    balance_distill = 'fc_balance_distill_' + args.distill_option + (str)(args.balance_factor)
    use_loss = 'CE_' + (str)(args.losssrc2)
    if args.balance_ce:
        use_loss += '_balance'

    args.root_model = os.path.join(teacher_path, 'student', distill_T, balance_distill, use_loss)
...
def train(train_loader, teacher_model, lws_model, student_enc, student_fc,
    ...
    for i, (images, target) in enumerate(train_loader):
        ...
        if not args.fp16:
            ...
            fc_loss = kl_loss(logits, teacher_out, cls_num_list=args.cls_num_list)
            if args.losssrc2:
                fc_loss *= 1 / 2
                fc_loss += criterion(logits, target) / 2
            ...
            if args.losssrc2:
                enc_loss *= 1 / 2
                enc_loss += criterion(logits, target) / 2
            ...
            with autocast():
                ...
                fc_loss = kd_kl_loss(logits, teacher_out, cls_num_list=args.cls_num_list)
                if args.losssrc2:
                    fc_loss *= 1 / 2
                    fc_loss += criterion(logits, target) / 2
                fc_optimizer.zero_grad()
                ...
                feats = student_enc(images)
                logits = student_fc(feats)
                enc_loss = kd_kl_loss(logits, teacher_out)
                if args.losssrc2:
                    enc_loss *= 1 / 2
                    enc_loss += criterion(logits, target) / 2
                enc_optimizer.zero_grad()
