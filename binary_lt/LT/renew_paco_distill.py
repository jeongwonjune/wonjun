...
parser.add_argument('--root_path', type=str, default='./runs/renew/distill/paco')
...
parser.add_argument('--distill_option', type=str,
                    choices=['student', 'teacher_student', None],
                    default=None)
...
def main():
    ...
    use_loss = (str)(args.losssrc2)
    if args.balance_ce:
        use_loss += '_balance'

    if args.balance_distill:
        use_loss += ('/balanced' + (str)(args.balance_factor))
    args.root_model = os.path.join(args.root_path, args.mark, use_loss)
    if args.distill_option:
        args.root_model = os.path.join(args.root_model, args.distill_option)
...
def main_worker(gpu, ngpus_per_node, args):
    ...
    if args.dataset == 'cub':
        ...
        train_dataset = ImbalanceCIFAR100(root=args.data, imb_type='exp', imb_factor=args.imb_ratio, rand_number=0,
                                          train=True, downoad=True, transform=transform_train_cifar)
    ...
    if args.balance_ce:
        criterion_ce = create_loss(cls_num_list=args.cls_num_list)
    else:
...
def train(train_loader, teacher_model, student_model,
    ...
    for i, (images, target) in enumerate(train_loader):
        ...
        if not args.fp16:
            ...
            total_loss = kl_loss(logits, teacher_out, cls_num_list=args.cls_num_list, option=args.distill_option)
        else:
            with autocast():
                logits = student_model(images)
                total_loss = kl_loss(logits, teacher_out, cls_num_list=args.cls_num_list, option=args.distill_option)
