...
parser.add_argument('--factor_init', default=0, type=float)
...
def main():
    ...
    args.mark = '_'.join([
        ...
        ('factor_init' + (str)(args.factor_init)),
        ('minimization' + (str)(args.minimization)),
    ])
    powernorm = 'powernorm_' + args.distill_option

    args.root_model = os.path.join(teacher_path, 'student/balance_factor_learning', powernorm, args.mark)
...
def main_worker(gpu, ngpus_per_node, args):
    ...
    fc_factor = balance_factor(init_val=args.factor_init)
    enc_factor = balance_factor(init_val=args.factor_init)
    fc_factor.W.requires_grad = True
    enc_factor.W.requires_grad = True
    ...
    for i, (images, target) in enumerate(train_loader):
        ...
        if not args.fp16:
            ...
            if i % 10 == 1:
                ...
                writer.add_scalar('enc factor', sigmoid(enc_factor.module.W).item(), epoch * iters + i)
                writer.add_scalar('fc factor', sigmoid(fc_factor.module.W).item(), epoch * iters + i)
            ...
            with autocast():
                ...
                fc_total_loss = fc_factor(fc_loss[None,:], fc_bal_loss[None,:])
                ...
                enc_total_loss = enc_factor(enc_loss, enc_bal_loss)
                ...
                if i % 10 == 1:
                    enc_factor_log.update(enc_factor, logits.size(0))
                    fc_factor_log.update(fc_factor, logits.size(0))
...
class balance_factor(nn.Module):
    def __init__(self, init_val=0.0):
        super(balance_factor, self).__init__()
