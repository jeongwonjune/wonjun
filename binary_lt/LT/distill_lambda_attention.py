...
parser.add_argument('--factor_init', default=0.0, type=float)
parser.add_argument('--maximization', action='store_true')
...
parser.add_argument('-d', '--balance_distill_layer', type=str, choices=['fc_only', 'enc_only', 'enc_fc'], default='enc_fc')
...
parser.add_argument('--sparse_update', type=int, default=None)
...
def main():
    ...
    assert (str)(args.imb_ratio) in teacher_path
    assert args.dataset in teacher_path

    minimize = False if args.maximization else True
    args.mark = '_'.join([
        ('epochs' + (str)(args.epochs)),
        ('lr' + (str)(args.lr)),
        ('bs' + (str)(args.batch_size)),
        ('factor_lr' + (str)(args.factor_lr)),
        ('minimization' + (str)(minimize)),
    ])
    ...
    if args.use_time:
        ...
        use_time = 'no_time'
    factor_option = '_'.join(['attention',
                              args.factor_ver,
                              use_time
                              ])
    # factor_option = 'factor_attention_' + args.factor_option
    if args.sparse_update:
        assert args.sparse_update > 1
        factor_option += '_sparse_update_' + (str)(args.sparse_update)

    args.root_model = os.path.join(teacher_path, 'student',
                                   ('attention_' + args.balance_distill_layer),
                                   factor_option,
                                   args.mark)

    if args.backup:
        assert os.path.exists(args.root_model)
        args.root_model = os.path.join(args.root_model, 'backup')
        os.makedirs(args.root_model, exist_ok=True)
        num_files = len(os.listdir(args.root_model))
        new_fol = 'try%02d' % num_files
        args.root_model = os.path.join(args.root_model, new_fol)
    os.makedirs(args.root_model, exist_ok=True)
...
def main_worker(gpu, ngpus_per_node, args):
    ...
    enc_factor = get_attention_module(version=args.factor_ver, use_time=use_time, gpu=args.gpu)
    fc_factor = get_attention_module(version=args.factor_ver, use_time=use_time, gpu=args.gpu)
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        if not args.multiprocessing_distributed or \
            ...
            writer.add_scalar('head acc', head_acc, epoch)
            writer.add_scalar('med acc', med_acc, epoch)
            writer.add_scalar('tail acc', tail_acc, epoch)
...
def train(train_loader, teacher_model, lws_model, student_enc, student_fc,
    ...
    enc_factor_log = AverageMeter('enc balanace factor', ':6.2f')
    fc_factor_log = AverageMeter('fc balanace factor', ':6.2f')
    ...
    sparse_freq = args.sparse_update
    for i, (images, target) in enumerate(train_loader):
        ...
        if not args.fp16:
            ...
            if 'fc' in args.balance_distill_layer:
                if args.use_time:
                    iter = iters * epoch + i
                    fc_total_loss = fc_factor(logits, teacher_out, time=iter, option=args.distill_option)
                elif args.use_time_norm:
                    iter = iters * epoch + i
                    iter /= iters * args.epochs
                    fc_total_loss = fc_factor(logits, teacher_out, time=iter, option=args.distill_option)
                else:
                    fc_total_loss = fc_factor(logits, teacher_out, time=None, option=args.distill_option)
            else:
                fc_total_loss = kd_kl_loss(logits, teacher_out)
            ...
            if args.maximization and ('fc' in args.balance_distill_layer):
                for p in fc_factor.parameters():
                    p.grad = p.grad * (-1)

            if args.sparse_update and ((i + 1) % sparse_freq == 0):
            ...
            if 'enc' in args.balance_distill_layer:
                if args.use_time:
                    iter = iters * epoch + i
                    enc_total_loss = fc_factor(logits, teacher_out, time=iter, option=args.distill_option)
                elif args.use_time_norm:
                    iter = iters * epoch + i
                    iter /= iters * args.epochs

                    enc_total_loss = enc_factor(logits, teacher_out, time=iter, option=args.distill_option)
                else:
                    enc_total_loss = enc_factor(logits, teacher_out, option=args.distill_option)
            else:
                enc_total_loss = kd_kl_loss(logits, teacher_out)
            ...
            if args.maximization and ('enc' in args.balance_distill_layer):
                for p in enc_factor.parameters():
                    if p.grad is not None:
                        p.grad = p.grad * (-1)

            if args.sparse_update and ((i + 1) % args.sparse_update == 0):
            ...
            if i % 10 == 1:
                if 'fc' in args.balance_distill_layer:
                    fc_factor_log.update(fc_factor.module.factors[1], logits.size(0))
                    writer.add_scalar('fc factor', fc_factor.module.factors[1], iters * epoch + i)
                if 'enc' in args.balance_distill_layer:
                    enc_factor_log.update(enc_factor.module.factors[1], logits.size(0))
                    writer.add_scalar('enc factor', enc_factor.module.factors[1], iters * epoch + i)
            ...
            with autocast():
                ...
                if 'fc' in args.balance_distill_layer:
                    if args.use_time:
                        ...
                        fc_total_loss = fc_factor(logits, teacher_out, option=args.distill_option)
                else:
                    fc_total_loss = kd_kl_loss(logits, teacher_out)
                ...
                scaler.scale(fc_total_loss).backward(retain_graph=True)
                ...
                enc_optimizer.zero_grad()
                enc_factor_optimizer.zero_grad()
                scaler.scale(enc_total_loss).backward()
        ...
        enc_losses.update(enc_total_loss.item(), logits.size(0))
        fc_losses.update(fc_total_loss.item(), logits.size(0))
...
#
# class enc_factor(nn.Module):
#     def __init__(self, init_val=0.0, gpu=None, num_classes=None):
#         super(enc_factor, self).__init__()
#         self.gpu = None
#         if gpu:
#             self.gpu = gpu
#
