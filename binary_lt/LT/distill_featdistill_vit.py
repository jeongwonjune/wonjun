...
from timm.models.efficientnet import tf_efficientnet_b7_ns, tf_efficientnetv2_m_in21k
#from models.reactnet_imagenet import reactnet as reactnet_imagenet, Classifier
from models.reactnet_optimized import Reactnet, Classifier
...
import timm
from models.bivit import VisionTransformer as bivit
...
parser.add_argument('--dataset', default='cub',
                    choices=['inat', 'places',
                             'cub', 'cifar10', 'cifar100',
                             'aircraft', 'caltech101', 'caltech256', 'cars',
                             'dogs', 'dtd', 'flowers'])
...
parser.add_argument('--student_model', help='choose model to use as a student', default='reactnet', type=str)
...
parser.add_argument('-p', '--print-freq', default=80, type=int,
...
def main():
    ...
    balance_distill = '_'.join(['balance_distill', args.balance_distill_layer, args.factor_ver])
    if args.use_time:
        use_time = 'use_time'
...
def main_worker(gpu, ngpus_per_node, args):
    ...
    if args.teacher_model == 'vit':    
        teacher_model = timm.create_model('vit_base_patch16_224')
        teacher_model.head = nn.Linear(teacher_model.head.in_features, args.num_classes)
    else:
    ...
    if args.student_model == 'reactnet':
        student_enc = Reactnet(num_classes=args.num_classes, ver2=True, penult_feat=True)
        student_fc = Classifier(num_classes=args.num_classes)
    else:
        student_enc = bivit( patch_size=16, embed_dim=768, depth=12, num_heads=12, mlp_ratio=4, qkv_bias=True, norm_layer=partial(nn.LayerNorm, eps=1e-6))
        student_fc = Classifier(feat_in = 768, num_classes = args.num_classes)
    
    if args.teacher_model == 'vit':
        feat_converter = transfer_conv(student_fc.fc.in_features, teacher_model.head.in_features)
    else:
    ...
    if args.distributed:
        ...
        if args.gpu is not None:
            ...
            student_enc = torch.nn.parallel.DistributedDataParallel(student_enc, device_ids=[args.gpu], find_unused_parameters=True)
            student_fc = torch.nn.parallel.DistributedDataParallel(student_fc, device_ids=[args.gpu], find_unused_parameters=True)
            teacher_model = torch.nn.parallel.DistributedDataParallel(teacher_model, device_ids=[args.gpu], find_unused_parameters=True)
            feat_converter = torch.nn.parallel.DistributedDataParallel(feat_converter, device_ids=[args.gpu], find_unused_parameters=True)
            enc_factor = torch.nn.parallel.DistributedDataParallel(enc_factor, device_ids=[args.gpu],find_unused_parameters=True)
            fc_factor = torch.nn.parallel.DistributedDataParallel(fc_factor, device_ids=[args.gpu],find_unused_parameters=True)
            if args.lws_model:
                lws_model = torch.nn.parallel.DistributedDataParallel(lws_model, device_ids=[args.gpu],find_unused_parameters=True)

            filename = f'{args.root_model}/ckpt.pth.tar'
            #if os.path.exists(filename):  args.resume = filename
    ...
    feat_loss = nn.KLDivLoss(reduction="batchmean")
...
def train(train_loader, teacher_model, lws_model, student_enc, student_fc,
    ...
    for i, (images, target) in enumerate(train_loader):
        ...
        if not args.fp16:
            s_out = student_enc(images)
            
            
            base = criterion(s_out,target)
            distill = F.kl_div(F.log_softmax(s_out, dim=1),  F.log_softmax(t_out, dim=1), reduction='sum', log_target=True )
            distill = kl_loss(s_out,t_out)
            enc_loss = 0.5*base + 0.5*distill
            enc_loss.backward()
            ...
            s_out = student_fc(s_out)
            #s_feat = s_feat.reshape(16, 768, 1, 1)
            #s_feat = feat_converter(s_feat)
            
            
            #enc_loss = kl_loss(s_out, t_out) + criterion(s_out, target)
            enc_loss = criterion(s_out, target)
        ...
        enc_kl_losses.update(enc_loss.item() if enc_loss is not 0 else 0, s_out.size(0))
        enc_feat_losses.update(base.item() if base is not 0 else 0, s_out.size(0))

        fc_kl_losses.update(distill.item() if distill is not 0 else 0, s_out.size(0))
