...
def main():
    ...
    args.root_model = os.path.join(teacher_path, 'student/attention_all',
                                   factor_option,
                                   args.mark)
    if args.backup:
...
def main_worker(gpu, ngpus_per_node, args):
    ...
    balance_factor_scheduler = torch.optim.lr_scheduler.CosineAnnealingLR(optimizer=balance_factor_optimizer,
                                                                          T_max=args.epochs)
...
def train(train_loader, teacher_model, lws_model, student_enc, student_fc,
    ...
    factor_log = AverageMeter('enc balanace factor', ':6.2f')
    ...
    for i, (images, target) in enumerate(train_loader):
        ...
        if not args.fp16:
            ...
            if args.use_time:
                iter = iters * epoch + i
                total_loss = balance_factor(logits, teacher_out, time=iter, option=args.distill_option)
            elif args.use_time_norm:
                iter = iters * epoch + i
                iter /= iters * args.epochs
                total_loss = balance_factor(logits, teacher_out, time=iter, option=args.distill_option)
            else:
                total_loss = balance_factor(logits, teacher_out, option=args.distill_option)
            ...
            with autocast():
                ...
                if args.sparse_update and ((i + 1) % args.sparse_update == 0):
                    scaler.step(balance_factor_optimizer)
                else:
                    scaler.step(balance_factor_optimizer)
                scaler.update()

        if i % 10 == 1:
            factor_log.update(balance_factor.module.factors[1], logits.size(0))
            writer.add_scalar('fc factor', balance_factor.module.factors[1], iters * epoch + i)
