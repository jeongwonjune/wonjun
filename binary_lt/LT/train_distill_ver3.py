...
def main_worker(gpu, ngpus_per_node, args):
    ...
    channel_converter = nn.Linear(in_features=128, out_features=1024)
    ...
    if args.distributed:
        ...
        if args.gpu is not None:
            ...
            channel_converter.cuda(args,gpu)
            ...
            student_fc = torch.nn.parallel.DistributedDataParallel(student_fc, device_ids=[args.gpu],
                                                                      find_unused_parameters=True)
            channel_converter = torch.nn.parallel.DistributedDataParallel(channel_converter, device_ids=[args.gpu],
                                                                          find_unused_parameters=True)
        ...
        channel_converter = channel_converter.cuda(args.gpu)
    ...
    cc_optimizer = torch.optim.Adam(channel_converter.parameters(), args.lr)
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        # train for one epoch
        train(train_loader, teacher_model, student_enc, student_fc, channel_converter,
...
def train(train_loader, teacher_model, student_enc, student_fc, channel_converter,
          enc_optimizer, fc_optimizer, cc_optimizer,
          epoch, scaler_enc, scaler_fc, scaler_cc,
          args):
    ...
    for i, (images, target) in enumerate(train_loader):
        ...
        if not args.fp16:
            ...
            teacher_feat = channel_converter(teacher_feat)
