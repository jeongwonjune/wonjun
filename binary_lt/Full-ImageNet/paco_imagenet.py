...
parser.add_argument('--root_path', type=str, default='./runs')
...
def main_worker(gpu, ngpus_per_node, args):
    ...
    model = moco.builder.MoCo(
        ...
        args.moco_dim, args.moco_k, args.moco_m, args.moco_t, args.mlp, args.normalize)
    print(model)
    ...
    criterion = PaCoLoss(alpha=args.alpha, beta=args.beta, gamma=args.gamma, temperature=args.moco_t, K=args.moco_k).cuda(args.gpu)
    ...
    if args.aug == 'regular':
       transform_train = moco.loader.TwoCropsTransform(transforms.Compose(augmentation_regular))
    elif args.aug == 'mocov2':
       transform_train = moco.loader.TwoCropsTransform(transforms.Compose(augmentation))
    elif args.aug == 'sim':
       transform_train = moco.loader.TwoCropsTransform(transforms.Compose(augmentation_sim))
    elif args.aug == 'randcls_sim':
       transform_train = moco.loader.TwoCropsTransformV2([transforms.Compose(augmentation_randncls), transforms.Compose(augmentation_sim)])
    elif args.aug == 'randclsstack_sim':
       transform_train = moco.loader.TwoCropsTransformV2([transforms.Compose(augmentation_randncls_stack), transforms.Compose(augmentation_sim)])
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        if not args.multiprocessing_distributed or \
                ...
                }, is_best, filename=f'{args.root_model}/moco_ckpt.pth.tar')
